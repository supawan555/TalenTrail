...
Runs two ways:
* In-process: on startup and then every 24h (local dev, Render). Controlled by
  settings.RUN_BACKGROUND_JOBS.
* Vercel Cron: calls GET /cron/auto-archive once a day (see routers/cron.py),
  because Vercel instances are short-lived and cannot hold a 24h loop.
"""
import asyncio
...
def archive_hired_candidates() -> int:
    """Archive candidates hired more than HIRED_ARCHIVE_DAYS ago; return how many."""
...
async def auto_archive_hired_candidates():
    """Background-safe wrapper: never lets an error kill the caller."""
    try:
        archive_hired_candidates()
...
def register_auto_archive(app: FastAPI) -> None:
    ...
    async def startup_auto_archive():
        ...
        if not settings.RUN_BACKGROUND_JOBS:
            print("[Auto-Archive] In-process loop disabled; expecting Vercel Cron to call /cron/auto-archive")
            return
